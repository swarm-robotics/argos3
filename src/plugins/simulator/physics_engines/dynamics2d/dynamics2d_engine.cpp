...
namespace argos {
   ...
   REGISTER_PHYSICS_ENGINE(CDynamics2DEngine,
                           ...
                           "the union of the rectangular area within the arena assigned to each engine must\n"
                           "cover the entire arena. Otherwise, robots can \"escape\" the configured physics\n"
                           "engines and cause a fatal exception (this is not an issue when a single physics\n"
                           "engine is used, because the engine covers the entire arena by default). To use\n"
                           "multiple physics engines, use the following syntax (all attributes are mandatory):\n\n"
                           ...
                           "counter-clockwise order).  In the above example, the physics engine is assigned\n"
                           "the square with lower left coordinates (0,0) and upper right coordinates (4, 4),\n"
                           "and verticle are specified as LL, LR, UR, UL counter-clockwise.\n\n"

                           "OPTIMIZATION HINTS\n\n"

                           "1. A single physics engine is generally sufficient for swarms <= 50 robots to obtain\n"
                           "   faster than real-time performance with optimized code. For larger swarms and/or large\n"
                           "   arenas multiple engines should be used for maximum performance.\n"
                           "2. Using multiple engines in such scenarios generally incurs more overhead than the\n"
                           "   performance benefits.\n"
                           "3. In general, using the same number of ARGoS threads as physics engines gives\n"
                           "   maximum performance (1-thread per engine per available CPU core).\n"
                           ""
